...
the_connection = mavutil.mavlink_connection(device='COM5', baud=57600) 
...
try:
    ...
    serialPort = serial.Serial(port='COM8', baudrate=115200, timeout=1) #115200
...
im.set_clim(vmin=40, vmax=100)  # rescale colors
...
def getFPV_data():
    ...
    while True: #time.time() < t_end:                   # COMMAND_ACK  #UNKNOWN_292 type='SENSOR_AVS', 
        ...
        return mel_intensity, azimuth_deg, active_intensity  
...
def updateSpectrogram():
    ...
    mel_intensity, azimuth_deg, active_intensity = getFPV_data()
    ...
    if active_intensity > 25:
        updateYaz = 1*np.cos(3.14/180*azimuth_deg)  #X #az 
        updateXaz = 1*np.sin(3.14/180*azimuth_deg)  #Y
    else:
        updateYaz = 1.25*np.cos(yaw)  #X #yaw
        updateXaz = 1.25*np.sin(yaw)  #Y
