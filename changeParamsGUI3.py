...
def getParams():
    ...
    for names in param_names:
        ...
        time.sleep(0.3) #0.01 #delay to ensure messages are received in order
...
def updatePlots():
    ...
    global background2, background3, background4, updateParams
    ...
    if updateParams:
        try: 
            act0 = float(updateParams['HAP_ACT_INT'].get())
        except ValueError:
...
def plotActiveIntensity():
    ...
    #intFig.canvas.draw()  # draw everything; necessary to update the canvas before blitting
...
actvButton = tk.Button(intFrame, text = 'PLOT ACTIVE INTENSITY', bg ='gray', fg= 'black', padx=5, pady=5, command= startActiveIntensity) #lambda: [startActiveIntensity(), startAz()])
actvButton.pack(side='bottom', pady=10, padx=10)
...
def runThreads():
    ...
    # # drain once here, remove the drain from getFPV_data and getSpecData
    # while connection.recv_match(blocking=False) is not None:
    #     pass
...
# roll =( math.degrees(roll)  +360) % 360 
...
# p = (math.degrees(roll) +360) % 360 #normalize roll
# print('roll:', p)
