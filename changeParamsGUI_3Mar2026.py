...
#root.geometry(f"{int(screen_width*.9)}" + 'x' + f"{int(screen_height*.8)}")
...
# create frame  #frame2
imgFrame = tk.Frame(master=tab2, padx=5, pady=5) #,padx=10, pady=10) master=visuals_row,
imgFrame.pack(side="left",padx=5)#grid(row=0, column=2)
...
circle_green = plt.Circle((0,0), 0.05, color='green')
...
def run(updateParams):
    ...
    for param_name, entry_field in updateParams.items():
        ...
        if ((param_name == 'HAP_SENSE_AVS_R') or (param_name == 'HAP_SENSE_AVS_L') or (param_name == 'HAP_SENSE_IMU')or(param_name == 'HAP_MULTIPLEX') or (param_name== 'HAP_DRV_EFFECT_T') or (param_name ==  'HAP_DRV_EFFECT_B')):
...
def getParams():
    ...
    for names in param_names:
        ...
        if ((message.param_id == 'HAP_SENSE_AVS_R') or (message.param_id == 'HAP_SENSE_AVS_L') or (message.param_id == 'HAP_SENSE_IMU') or (message.param_id == 'HAP_MULTIPLEX') or (message.param_id == 'HAP_DRV_EFFECT_T') or (message.param_id ==  'HAP_DRV_EFFECT_B')) and (message.param_type == 6):
...
def updatePlots():
    ...
    global ind,background2, background3, updateParams
    global updateXaz_r, updateYaz_r, updateXaz_l, updateYaz_l,updateYroll, updateXroll
    ...
    if id == 44:
        ...
        updateYroll = 1*np.cos(np.deg2rad(roll))  #X 
        updateXroll = 1*np.sin(np.deg2rad(roll))  #Y
    ...
    #yaw = (math.degrees(yaw_transformed)+360) % 360 #normalize yaw
    #print(math.degrees(yaw_transformed))
    ...
    # updateYroll = 1*np.cos(roll)  #X 
    # updateXroll = 1*np.sin(roll)  #Y

    # np.deg2rad(az)
    # updateYaz = 1*np.cos(np.deg2rad(az))  #X #az 
    # updateXaz = 1*np.sin(np.deg2rad(az))  #Y
    ...
    circle_red2.set_center((updateXroll, updateYroll))
